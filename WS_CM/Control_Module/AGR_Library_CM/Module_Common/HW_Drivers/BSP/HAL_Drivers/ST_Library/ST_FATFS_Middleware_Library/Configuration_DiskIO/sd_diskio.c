...
/* SDIO Port Definition */
#define BSP_SD_PORT		   BSP_SD1
...
//#define SD_DEFAULT_BLOCK_SIZE 4096
#define SD_DEFAULT_BLOCK_SIZE 1024
...
#endif
  {
	  //if (BSP_SD_GetCardState() == SD_TRANSFER_OK)  //origin
	  if (BSP_GetSDState(BSP_SD_PORT) == BSP_SD_TRANSFER_OK)
	  {
		  return 0;
	  }
  }
...
Stat = STA_NOINIT;
...
#endif
  {
    ...
    //if(BSP_SD_Init() == MSD_OK)  //origin
	if(BSP_InitSD(BSP_SD_PORT) == BSP_MSD_OK)
    {
...
DRESULT SD_read(BYTE lun, BYTE *buff, DWORD sector, UINT count)
{
  ...
  */
  ...
  if (!((uint32_t)buff & 0x3))
  {
    ...
    //ret = BSP_SD_ReadBlocks_DMA((uint32_t*)buff, (uint32_t)(sector), count); //origin
	ret = BSP_ReadSDBlockDMA(BSP_SD_PORT,(uint32_t*)buff, (uint32_t)(sector), count);

    //if (ret == MSD_OK) {  //origin
	if (ret == BSP_OK) {
#if (osCMSIS < 0x20000U)
    ...
    else
    {
      ...
      for (i = 0; i < count; i++)
      {
    	  ...
    	  //if (ret == MSD_OK )
    	  ret = BSP_ReadSDBlockDMA(BSP_SD_PORT,(uint32_t*)scratch, (uint32_t)sector++, 1);
            ...
            if (event.value.v == READ_CPLT_MSG)
            {
              ...
              if ((status == osOK) && (event == READ_CPLT_MSG))
              {
...
#endif
                {
                  ...
                  ret = BSP_GetSDState(BSP_SD_PORT);
...
DRESULT SD_write(BYTE lun, const BYTE *buff, DWORD sector, UINT count)
{
  ...
  */
  ...
  if(BSP_WriteSDBlockDMA(BSP_SD_PORT, (uint32_t*)buff, (uint32_t) (sector), count) == BSP_OK)
  {
#if (osCMSIS < 0x20000U)
     ...
     */
     SCB_InvalidateDCache_by_Addr((uint32_t*)scratch, BLOCKSIZE);
      ...
      for (i = 0; i < count; i++)
      {
        ...
        ret = BSP_WriteSDBlockDMA(BSP_SD_PORT,(uint32_t*)scratch, (uint32_t)sector++, 1);
        if (ret == BSP_MSD_OK )
        {
          /* wait until the read is successful or a timeout occurs */
#if (osCMSIS < 0x20000U)
...
{
  DRESULT res = RES_ERROR;
  //BSP_SD_CardInfo CardInfo;     //origin
  BSP_SD_CardInfoTypeDef_t CardInfo;
  ...
  case GET_SECTOR_COUNT :
	...
	BSP_GetSDInfo(BSP_SD_PORT, &CardInfo);
