...
static void InitLPF(WIDM_LPFState* state, double currentInput);
...
/* Low Pass Filters */
/**
 * @brief Initializes the low-pass filter state.
 * 
 * This function initializes the low-pass filter state with the current input value
 * and marks the filter as initialized.
 * 
 * @param state Pointer to the filter state structure.
 * @param currentInput The current input signal to initialize the filter state with.
 */
static void InitLPF(WIDM_LPFState* state, double currentInput)
{
    state->prevOutput = currentInput; // Set initial output to current input to avoid startup transients
    state->isInitialized = true;      // Mark the filter as initialized
}

/**
 * @brief Updates the low-pass filter state with a new input and custom smoothing factor.
 * 
 * This function applies the low-pass filter to the input signal, using a custom smoothing factor
 * to control the degree of smoothing. It smooths out high-frequency fluctuations and updates
 * the filter's state.
 * 
 * @param state Pointer to the filter state structure.
 * @param currentInput The new input signal to be processed.
 * @param alpha The smoothing factor for the low-pass filter, controlling the filter's responsiveness.
 * @return The filtered signal output.
 */
double UpdateLPF_CustomAlpha(WIDM_LPFState* state, double currentInput, double alpha)
{
    if (!state->isInitialized) {
        InitLPF(state, currentInput);
    }

    // Apply the low-pass filter equation with custom alpha
    double currentOutput = alpha * currentInput + (1 - alpha) * state->prevOutput;

    // Update the state for the next iteration
    state->prevOutput = currentOutput;
...
double WIDM_BPF_Peak_Prof_1_RH(double r, double w)
{
	...
	static double y1_2_RH, y2_2_RH, y3_2_RH, r1_2_RH, r2_2_RH, r3_2_RH;
...
double WIDM_BPF_Peak_Prof_1_LH(double r, double w)
{
	...
	static double y1_2_LH, y2_2_LH, y3_2_LH, r1_2_LH, r2_2_LH, r3_2_LH;
...
double WIDM_BPF_Peak_Prof_2_RH(double r, double w)
{
	...
	static double y1_3_RH, y2_3_RH, y3_3_RH, r1_3_RH, r2_3_RH, r3_3_RH;
...
double WIDM_BPF_Peak_Prof_2_LH(double r, double w)
{
	...
	static double y1_3_LH, y2_3_LH, y3_3_LH, r1_3_LH, r2_3_LH, r3_3_LH;
...
double WIDM_LPF_1NE(double r, double w)
{
	...
	static double y1_LPF_1ne;
	static double r1_LPF_1ne;
	double T_1ne = 0.001;
	double K_1ne = 5;
