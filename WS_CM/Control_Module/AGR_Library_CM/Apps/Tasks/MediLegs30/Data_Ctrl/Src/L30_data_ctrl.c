...
IOIF_USBState usb_host_init = IOIF_USB_FAIL;
IOIF_USB_Status_t usb_test_init_res = IOIF_FATFS_USB_FAIL;
...
uint8_t file_ConsecutiveWrite;//sdio transmit sync semaphore
...
void InitDataCtrl(void)
{
	...
	usb_host_init = IOIF_InitUSB(IOIF_USBH_MSC, IOIF_USB_TIM_NULL);

	usb_test_init_res = IOIF_FATFS_USB_Init(IOIF_FATFS_USB_MSC, (uint8_t*) "0:");
	...
	uint8_t t_temp=0;
		// /* Data Storage Test : 20240128 */
		 ...
		 if (mount_res == FR_OK && file_ConsecutiveWrite == false)
		 	 	open_res = IOIF_FileOpenCreateAppend(&file_t, (uint8_t*)"0:SUIT_H10_GAIT_DATA_02.txt");
